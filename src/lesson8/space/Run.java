...
В методе первым делом осуществляем предстартовую проверку переданного объекта, если она провалилась(метод вернул false) выводим сообщение «Предстартовая проверка провалена». Если проверка прошла успешно, то производим запуск двигателей(вызываем метод запуска двигателя).
...
public class Run {
    public static void main(String[] args) throws InterruptedException {
        ...
        SpaceY spaceY = new SpaceY();

        spacePort.launch(shuttle);
        System.out.println("****************");
        spacePort.launch(spaceX);
        System.out.println("****************");
        spacePort.launch(spaceY);
    }
}
